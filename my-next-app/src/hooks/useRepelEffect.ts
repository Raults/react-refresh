...
export const useRepelEffect = (
    ...
    useEffect(() => {
        ...
        if (enabled) {
            const existingSpans = container.querySelectorAll("span[data-repel-char]");
            ...
            // Unwrap on disable
            const spans = container.querySelectorAll("span[data-repel-word]");
            spans.forEach((span) => {
                const textNode = document.createTextNode(span.textContent || "");
                span.replaceWith(textNode);
            });
